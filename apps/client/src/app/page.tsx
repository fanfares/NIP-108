...
export default function Home() {
  ...
  useEffect(() => {
    const newRelay = relayInit("wss://dev.nostrplayground.com")
    newRelay.on("connect", () => {
      setRelay(newRelay)
    })
    newRelay.connect()
      ...
      <div className="mt-5">
        ...
        <div className="mt-4 flex justify-center">
          <button
            ...
            className={`px-3 py-2 border border-white rounded-full text-white hover:bg-white hover:text-black hover:border-black w-48 first-letter:text-2xl items-center text-start transition-all duration-300`}>
            {gateLoading && gatedNote.note.id === gateLoading
              ? "🔓 Unlocking..."
              : `🔒 Unlock: ${(gatedNote.cost / 1000).toFixed(0)} ⚡ Sats`}
          </button>
        </div>
      </div>
    )
